...
def prepare_symbol_dataset(exchange, symbol: str, timeframes: list, indicators: list, news_cache=None):
    ...
    if requested_timeframes:
        ...
        log(f"⚠️ {symbol}: модель не указала таймфреймы, используем fallback {TIMEFRAME}", Fore.YELLOW)
    seen_cols = set()
...
def ai_select_portfolio(exchange, symbols, positions_map, equity, available_margin):
    ...
    system_msg = """You act as a risk-neutral intraday portfolio manager for Bybit USDT perpetual swaps. Use available_pairs (and you may propose other USDT contracts if they seem attractive; the bot will attempt to gather data). indicator_pool and timeframe_pool list the default metrics; request any extra indicators or timeframes via the needs field when required. Always schedule the bot's next self-run instead of relying on cron. Respond strictly in JSON of the form:
{
  "targets": [
    {
      "symbol": "PAIR",
...
Always provide next_run_minutes as a positive number so the bot can sleep until the next cycle; include next_run_time only when you can justify an absolute UTC slot (use ISO 8601 with Z). If you cannot decide, return {\"targets\": [], \"needs\": [\"news\"...], \"reason\": \"...\"}."""
    messages = [
...
def execute_extra_orders(exchange, symbol, orders, current_position=None, open_orders=None, fallback_amount=None):
    executed = []
...
def run_cycle():
    ...
    if not IS_LATEST_VERSION:
        fallback_notice = f"⚠️ Running fallback build {version_label}."
    ...
    for i,sym in enumerate(symbols_sequence,1):
        ...
        try:
            ...
            if not requested_timeframes:
                log(f'⚠️ {sym}: модель не указала таймфреймы, используем fallback {TIMEFRAME}', Fore.YELLOW)
    ...
    if selection_next_run is not None:
        ...
        if minutes_val and minutes_val > 0:
            ...
            next_msg = (
                f"🔁 Следующий запуск через {minutes_val:.0f} мин (~{next_local.strftime('%Y-%m-%d %H:%M:%S %Z')})"
            )
            log(next_msg, Fore.CYAN)
            send_tg(next_msg)
        else:
            log("⚠️ Модель вернула некорректное next_run_minutes.", Fore.YELLOW)
        ...
        if parsed_time:
            ...
            try:
                ...
                if delta > 0:
                    ...
                    msg = f"🔁 Следующий запуск по модели: {next_local.strftime('%Y-%m-%d %H:%M:%S %Z')}"
                    log(msg, Fore.CYAN)
                    send_tg(msg)
                else:
                    log("⚠️ Модель указала прошедшее время next_run_time.", Fore.YELLOW)
            except Exception as exc:
                log(f"⚠️ Не удалось разобрать next_run_time '{selection_next_time}': {exc}", Fore.YELLOW)
    if next_delay_minutes is None:
        next_delay_minutes = DEFAULT_NEXT_RUN_MINUTES
        fallback_msg = f"🔁 Используем интервал по умолчанию {next_delay_minutes:.0f} мин."
    ...
    if not IS_LATEST_VERSION:
        log(f"⚠️ Running fallback build {version_label_end}.", Fore.YELLOW)
    send_tg(f"═══════════ END SESSION {end_stamp} ═══════════")
    send_tg(f"🏁 {end_balance_text}")
    info_tail = CHANGELOG_TEXT if CHANGELOG_TEXT else "No changelog entry"
    send_tg(f"ℹ️ Версия {version_label_end}. {info_tail}")
...
def main():
    ...
    while True:
        try:
            ...
            log("⏹ Остановлено пользователем.", Fore.YELLOW)
            break
        except Exception:
            raise
