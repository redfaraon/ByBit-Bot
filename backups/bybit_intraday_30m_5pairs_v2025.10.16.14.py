...
BOT_CHANGELOG = (
    "- Добавил управление упоминанием: новая глобальная конфигурация TELEGRAM_MENTION и детектор реального счёта (IS_REAL_ACCOUNT) в bybitbot.py:15-17 и bybitbot.py:724-763. Helper send_trade_update (bybitbot.py:868-873) теперь префиксует сообщения только на "
"боевом аккаунте; именно он используется там, где реально меняется состояние у брокера (например, отмены на bybitbot.py:1528, рыночные закрытия bybitbot.py:2261, открытия bybitbot.py:2280, отчёт об исполненных доп. ордерах bybitbot.py:664/2409). Информационные " 
"сообщения по‑прежнему идут без упоминаний через send_tg."
"- Усилил разделители сессий: на старте и завершении main() пишем баннеры с версией и changelog в лог (bybitbot.py:2031-2036, bybitbot.py:2429-2434) и отправляем те же маркеры в Telegram вместе с балансом."
)
...
def execute_extra_orders(exchange, symbol, orders, current_position=None, open_orders=None):
    ...
    amount_min = to_float(amount_limits.get("min")) if isinstance(amount_limits, dict) else None
    amount_max = to_float(amount_limits.get("max")) if isinstance(amount_limits, dict) else None
    min_cost = to_float(cost_limits.get("min")) if isinstance(cost_limits, dict) else None
    ticker_cache = None
    ...
    for idx, order in enumerate(orders, 1):
        ...
        position_amount = to_float(current_position.get("amount")) if current_position else 0.0
        if params.get("reduceOnly"):
        ...
        if ccxt_type in ("stop", "stoplimit", "stopLimit", "takeProfit", "stopLoss", "trailingStop"):
            ...
            if not trigger_dir:
                ...
                if isinstance(ticker_cache, dict):
                    last_price = to_float(ticker_cache.get("last") or ticker_cache.get("close"))
                if last_price is None and current_position:
                    last_price = to_float(current_position.get("entryPrice"))
