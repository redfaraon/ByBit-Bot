...
BOT_VERSION = "2025.10.16.11"
BOT_CHANGELOG = (
    "bybitbot.py:149 добавил утилиты _apply_indicator_to_df, _serialize_df и prepare_symbol_dataset, чтобы быстро собирать свечи и индикаторы по набору таймфреймов, которые попросит модель."
" bybitbot.py:236 реализовал стадию отбора инструментов (ai_select_portfolio) и вспомогательные сборщики build_portfolio_bundle/augment_bundle_with_needs, так что шаг 1 запроса к ИИ теперь решает, какие пары, таймфреймы и индикаторы анализировать на сессию."
"bybitbot.py:1375 расширил ai_decision: функция принимает дополнительный контекст (портфельные веса, выбранные таймфреймы, новости) и прокидывает его в промпт, сохранив прежнюю обработку needs."
"bybitbot.py:1790 переписал основной цикл: сначала вызываем ai_select_portfolio, затем подтягиваем нужные таймфреймы/индикаторы/новости по выбранным монетам, передаем их в ai_decision, и только если подбор не удался — откатываемся к исходному PAIR_LIST."
)
...
(доступно: higher_tf:<tf>, funding, open_interest, news), и только после доп. проверки выбирай конечное действие. 
