...
BOT_CHANGELOG = (
    "Расширен торговый универсум и индикаторы по умолчанию; добавлены PAIR_UNIVERSE/INDICATOR_POOL/DEFAULT_TIMEFRAMES с поддержкой выборок вне дефолта.",
    "Доп. контекст по needs теперь включает запрошенные таймфреймы; баннеры START/END используют LOG_TIMEZONE, а исполнение ордеров проверяет precision через safe_float.",
)
