...
BOT_VERSION = os.getenv("BYBITBOT_CHANGELOG_VERSION", "0.0.0.0")
CHANGELOG_TEXT = os.getenv("BYBITBOT_CHANGELOG_TEXT", "")
EXPECTED_VERSION = os.getenv("BYBITBOT_EXPECTED_VERSION", BOT_VERSION)
IS_LATEST_VERSION = BOT_VERSION == EXPECTED_VERSION
__doc__ = (
    "Bybit Intraday AI Trading Bot (30m, 5 pair USDT perpetual)\n"
    f"Version: {BOT_VERSION}\n"
    "Powered by OpenAI GPT and Telegram integration."
)

# Change log: stored in CHANGELOG.txt for safe editing/copy
CHANGELOG_FILE = Path(__file__).with_name("CHANGELOG.txt")
if not CHANGELOG_TEXT:
    try:
        CHANGELOG_TEXT = CHANGELOG_FILE.read_text(encoding="utf-8").strip()
    except FileNotFoundError:
        CHANGELOG_TEXT = ""


def apply_metadata(version: str, changelog: str, expected: str | None = None) -> None:
    """Update module metadata so loader defines the active version."""
    global BOT_VERSION, CHANGELOG_TEXT, EXPECTED_VERSION, IS_LATEST_VERSION, __doc__
    BOT_VERSION = version
    CHANGELOG_TEXT = changelog.strip()
    EXPECTED_VERSION = expected or BOT_VERSION
    IS_LATEST_VERSION = BOT_VERSION == EXPECTED_VERSION
    __doc__ = (
        "Bybit Intraday AI Trading Bot (30m, 5 pair USDT perpetual)\n"
        f"Version: {BOT_VERSION}\n"
...
CHANGELOG_BROADCASTED = False

# Fallback pools used when environment overrides are absent
...
DEFAULT_INDICATOR_CANDIDATES = ["ema20", "ema50", "rsi14", "atr14", "volume"]
DEFAULT_TIMEFRAME_CANDIDATES = ["30m", "4h"]
...
UNIVERSE_REFRESH_INTERVAL_MINUTES = 360.0
PAIR_LIST_FROM_ENV = False
PAIR_UNIVERSE_FROM_ENV = False
INDICATOR_POOL_FROM_ENV = False
TIMEFRAMES_FROM_ENV = False

try:
    UNIVERSE_CACHE = json.loads(UNIVERSE_CACHE_FILE.read_text(encoding="utf-8"))
except Exception:
    UNIVERSE_CACHE = {}

apply_metadata(BOT_VERSION, CHANGELOG_TEXT, EXPECTED_VERSION)
...
CONFIG_OVERRIDE_MAP = {
    ...
    "default_next_run_minutes": "DEFAULT_NEXT_RUN_MINUTES",
    "trailing_atr_mult": "TRAILING_ATR_MULT",
}

LIST_OVERRIDE_MAP = {
    "pair_list": ("PAIR_LIST", "PAIR_LIST_FROM_ENV", False),
    "pair_universe": ("PAIR_UNIVERSE", "PAIR_UNIVERSE_FROM_ENV", False),
    "indicator_pool": ("INDICATOR_POOL", "INDICATOR_POOL_FROM_ENV", True),
    "default_timeframes": ("DEFAULT_TIMEFRAMES", "TIMEFRAMES_FROM_ENV", False),
}


def sanitize_config_override(key: str, value: Any):
    try:
        if key == "risk_pct":
            val = float(value)
            if val <= 0:
                return None
            return max(0.0001, min(val, 0.05))
        if key == "leverage":
            val = float(value)
            if val <= 0:
                return None
            return max(1.0, min(val, 100.0))
        if key in ("sl_atr", "tp_atr"):
            val = float(value)
            if val <= 0:
                return None
            return max(0.1, min(val, 10.0))
        if key == "order_margin_utilization":
            val = float(value)
            return max(0.1, min(val, 1.0))
        if key == "max_open_positions":
            val = int(float(value))
            if val < 0:
                return 0
            return min(val, 20)
        if key == "default_next_run_minutes":
            val = float(value)
            if val <= 0:
                return None
            return max(1.0, min(val, 360.0))
        if key == "trailing_atr_mult":
            val = float(value)
            if val < 0:
                return 0.0
            return min(val, 10.0)
...
def normalize_generic_list(value: Any, to_lower: bool = False) -> Optional[list[str]]:
    if value is None:
        return None
    items: list[str] = []
    if isinstance(value, str):
        ...
        for part in parts:
            cleaned = part.strip()
            if cleaned:
                items.append(cleaned.lower() if to_lower else cleaned)
...
def sanitize_symbol_overrides(raw: Any) -> dict[str, float]:
    ...
    for key, value in raw.items():
        ...
        if key_lower in CONFIG_OVERRIDE_MAP:
            sanitized = sanitize_config_override(key_lower, value)
            if sanitized is not None:
                cleaned[key_lower] = sanitized
    return cleaned


def sanitize_symbol_config_map(raw: Any) -> dict[str, dict[str, float]]:
    """Return a symbol -> sanitized overrides mapping."""
    if not isinstance(raw, dict):
        return {}
    ...
    for key, value in raw.items():
        ...
        if not symbol:
            continue
        overrides = sanitize_symbol_overrides(value)
        if not overrides:
            continue
...
def apply_config_overrides_from_selection(selection: dict | None):
    if not selection or not isinstance(selection, dict):
        return None
    config_section = selection.get("config") or selection.get("configuration") or {}
    if not isinstance(config_section, dict):
        return None
    use_defaults = config_section.get("use_defaults")
    if isinstance(use_defaults, str):
        use_defaults = use_defaults.strip().lower() in ("1", "true", "yes", "on")
    if use_defaults:
        return None

    overrides_numeric: dict[str, Any] = {}
    overrides_lists: dict[str, Any] = {}
    raw_overrides = config_section.get("overrides")
    if isinstance(raw_overrides, dict):
        for key, value in raw_overrides.items():
            key_lower = key.strip().lower() if isinstance(key, str) else key
            if key_lower in CONFIG_OVERRIDE_MAP:
                overrides_numeric[key_lower] = value
            elif key_lower in LIST_OVERRIDE_MAP:
                overrides_lists[key_lower] = value
    else:
    ...
    lists_node = config_section.get("lists") or config_section.get("portfolio")
    if isinstance(lists_node, dict):
        for key, value in lists_node.items():
            key_lower = key.strip().lower() if isinstance(key, str) else key
            if key_lower in LIST_OVERRIDE_MAP:
                overrides_lists[key_lower] = value

    applied_numeric: dict[str, Any] = {}
    for key, raw_value in overrides_numeric.items():
        global_name = CONFIG_OVERRIDE_MAP.get(key)
        if not global_name:
            continue
        sanitized = sanitize_config_override(key, raw_value)
        if sanitized is None:
            continue
        current_value = globals().get(global_name)
        if current_value == sanitized:
            continue
        globals()[global_name] = sanitized
        applied_numeric[key] = sanitized

    applied_lists: dict[str, list[str]] = {}
    for key, raw_value in overrides_lists.items():
        mapping = LIST_OVERRIDE_MAP.get(key)
        if not mapping:
            continue
        global_name, env_flag, to_lower = mapping
        normalized = normalize_generic_list(raw_value, to_lower=to_lower)
        if not normalized:
            continue
        globals()[global_name] = normalized
        if env_flag:
            globals()[env_flag] = False
        applied_lists[key] = normalized

    messages = []
    justification = config_section.get("justification") or config_section.get("reason")
    if applied_numeric:
        summary = ", ".join(f"{key}={value}" for key, value in applied_numeric.items())
        messages.append(f"Config overrides applied: {summary}")
    if applied_lists:
        summary = ", ".join(f"{key}={', '.join(vals)}" for key, vals in applied_lists.items())
        messages.append(f"Universe overrides applied: {summary}")
    if messages:
        message = " | ".join(messages)
        if justification:
            message += f" - {justification}"
        log(message, Fore.CYAN)
        send_tg(message)

    applied: dict[str, Any] = {}
    applied.update(applied_numeric)
    applied.update(applied_lists)
...
def _write_runtime_status(next_delay_minutes: float | None, next_run_dt: datetime.datetime | None, status: str) -> None:
    payload = {
        "status": status,
    ...
    if next_run_dt is not None:
        ...
        try:
            payload["next_run_local"] = next_run_dt.astimezone(_current_local_tz()).isoformat()
        except Exception:
            pass
    try:
...
def _save_universe_cache(cache: dict[str, Any]) -> None:
    global UNIVERSE_CACHE
    UNIVERSE_CACHE = cache
    try:
        UNIVERSE_CACHE_FILE.write_text(json.dumps(cache, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass


def _apply_cached_universe_settings() -> None:
    cache = UNIVERSE_CACHE or {}
    if not cache:
        return
    global PAIR_LIST, PAIR_UNIVERSE, INDICATOR_POOL, DEFAULT_TIMEFRAMES
    if not PAIR_LIST_FROM_ENV:
        pairs = cache.get("pairs") or cache.get("pair_list")
        if pairs:
            PAIR_LIST = [p for p in dict.fromkeys(pairs) if p]
    if not PAIR_UNIVERSE_FROM_ENV:
        universe = cache.get("pair_universe") or cache.get("pairs") or cache.get("pair_list")
        if universe:
            PAIR_UNIVERSE = [p for p in dict.fromkeys(universe) if p]
    if not INDICATOR_POOL_FROM_ENV:
        indicators = cache.get("indicators") or []
        if indicators:
            INDICATOR_POOL = [s.strip().lower() for s in indicators if s]
    if not TIMEFRAMES_FROM_ENV:
        timeframes = cache.get("timeframes") or []
        if timeframes:
            DEFAULT_TIMEFRAMES = [tf.strip() for tf in timeframes if tf]
...
def prepare_symbol_dataset(exchange, symbol: str, timeframes: list, indicators: list, news_cache=None):
    ...
    indicator_candidates = [ind.strip().lower() for ind in (indicators or []) if ind]
    if not indicator_candidates and INDICATOR_POOL:
        indicator_candidates = [ind for ind in INDICATOR_POOL if ind]
    requested_timeframes = [tf.strip() for tf in (timeframes or []) if tf]
    if requested_timeframes:
        timeframes = list(dict.fromkeys(requested_timeframes))
    elif DEFAULT_TIMEFRAMES:
        timeframes = list(dict.fromkeys(DEFAULT_TIMEFRAMES))
    else:
        timeframes = [TIMEFRAME]
        log(f"?? {symbol}: ������ �� ������� ����������, ���������� fallback {TIMEFRAME}", Fore.YELLOW)
    seen_cols = set()
...
def _condense_news_digest(news_digest, symbol_limit: int = 6, item_limit: int = 2):
    if not isinstance(news_digest, dict):
        return {}
    condensed = {}
    for idx, (sym, payload) in enumerate(news_digest.items()):
        if idx >= symbol_limit:
            break
        entry = {}
        if isinstance(payload, dict):
            ...
            if isinstance(summary, str) and summary.strip():
                entry["summary"] = summary.strip()[:320]
...
def maybe_refresh_universe(client: OpenAI, news_digest: dict | None) -> list[str] | None:
    if not AI_KEY:
        return None
    if PAIR_LIST_FROM_ENV and INDICATOR_POOL_FROM_ENV and TIMEFRAMES_FROM_ENV:
        return None
    global PAIR_LIST, PAIR_UNIVERSE, INDICATOR_POOL, DEFAULT_TIMEFRAMES
    cache = UNIVERSE_CACHE or {}
    last_updated = _parse_iso_datetime(cache.get("updated_at"))
    now = datetime.datetime.now(datetime.timezone.utc)
    if last_updated and (now - last_updated).total_seconds() < UNIVERSE_REFRESH_INTERVAL_MINUTES * 60:
        return None

    payload = {
        "current_pairs": PAIR_UNIVERSE or PAIR_LIST or DEFAULT_PAIR_CANDIDATES,
        "current_indicators": INDICATOR_POOL or DEFAULT_INDICATOR_CANDIDATES,
        "current_timeframes": DEFAULT_TIMEFRAMES or DEFAULT_TIMEFRAME_CANDIDATES,
        "news_digest": _condense_news_digest(news_digest or {}),
    }
    system_msg = (
        "You act as a crypto macro scout updating the default pair/indicator/timeframe pools for an intraday Bybit USDT perpetual bot. "
        ...
        "}\n"
        "If you cannot improve, repeat the current lists."
    )
    try:
    ...
    def _clean_list(values, lower=False):
        result = []
    ...
    new_pairs = _clean_list(suggestion.get("pairs"), lower=False)
    new_indicators = _clean_list(suggestion.get("indicators"), lower=True)
    new_timeframes = _clean_list(suggestion.get("timeframes"), lower=False)
    if not new_pairs:
        return None
    if not new_indicators:
        new_indicators = INDICATOR_POOL or DEFAULT_INDICATOR_CANDIDATES
    if not new_timeframes:
        new_timeframes = DEFAULT_TIMEFRAMES or DEFAULT_TIMEFRAME_CANDIDATES

    cache_payload = {
        "pairs": new_pairs,
        "pair_universe": new_pairs,
    ...
    if isinstance(notes, str) and notes.strip():
        cache_payload["notes"] = notes.strip()
    _save_universe_cache(cache_payload)

    if not PAIR_LIST_FROM_ENV:
        PAIR_LIST = list(new_pairs)
    if not PAIR_UNIVERSE_FROM_ENV:
        PAIR_UNIVERSE = list(new_pairs)
    if not INDICATOR_POOL_FROM_ENV and new_indicators:
        INDICATOR_POOL = list(new_indicators)
    if not TIMEFRAMES_FROM_ENV and new_timeframes:
        DEFAULT_TIMEFRAMES = list(new_timeframes)

    log(
        f"?? ��������� ��������� ������: {len(new_pairs)} ���, {len(new_indicators)} �����������, {len(new_timeframes)} �����������.",
        Fore.CYAN
    )
    if notes:
        log(f"?? ����������� ����������: {notes.strip()}", Fore.LIGHTBLACK_EX)
...
def ai_select_portfolio(exchange, symbols, positions_map, equity, available_margin):
    ...
    updated_symbols = maybe_refresh_universe(client, news_digest)
    if updated_symbols:
        symbols = list(dict.fromkeys(updated_symbols))
    positions_compact = []
    for sym in symbols:
    ...
    system_msg = (
    "You act as a risk-neutral intraday portfolio manager for Bybit USDT perpetual swaps. Use available_pairs (and you may propose other USDT contracts if they seem attractive; the bot will attempt to gather data). indicator_pool and timeframe_pool list the default metrics; request any extra indicators or timeframes via the needs field when required. Always schedule the bot's next self-run instead of relying on cron. config_defaults lists the current risk parameters from the .env; only override them when you can justify a materially better setting. Respond strictly in JSON of the form:\n"
    '{\n'
    '  "targets": [\n'
    '    {\n'
    '      "symbol": "PAIR",\n'
    ...
    '  "next_run_minutes": float,  # >0 minutes until the next cycle; prefer sensible 5/15/30 multiples\n'
    '  "next_run_time": "2025-10-16T12:00:00Z",  # optional precise UTC time if needed instead of minutes\n'
    '  "config": {\n'
    '    "use_defaults": true,\n'
    '    "overrides": {\n'
    '      "leverage": float,\n'
    ...
    '      "sl_atr": float,\n'
    '      "tp_atr": float,\n'
    '      "order_margin_utilization": float,\n'
    '      "max_open_positions": int,\n'
    '      "default_next_run_minutes": float\n'
    '    },\n'
    '    "justification": "why overrides are beneficial"\n'
    '  },\n'
    '  "symbol_configs": {\n'
    '    "PAIR": {\n'
    '      "leverage": float,\n'
    ...
    '  }\n'
    '}\n'
    "Always provide next_run_minutes as a positive number so the bot can sleep until the next cycle; include next_run_time only when you can justify an absolute UTC slot (use ISO 8601 with Z). Only override config_defaults when you have clear reasons; otherwise set use_defaults to true. Include every active symbol (selected targets and open positions) in symbol_configs with the settings you want applied; omit keys to keep the default values. "
    'If you cannot decide, return {"targets": [], "symbol_configs": {}, "needs": ["news"...], "reason": "..."}.'
)
    messages = [
        {"role": "system", "content": system_msg},
    ...
    try:
        ...
        log(f"⚠️ OpenAI portfolio select timeout: {exc}", Fore.YELLOW)
        send_tg(f"⚠️ OpenAI portfolio select timeout: {exc}")
        return None
    except Exception as exc:
...
def refresh_settings():
    load_environment()
    global PAIR_LIST, PAIR_UNIVERSE, INDICATOR_POOL, DEFAULT_TIMEFRAMES, DEFAULT_NEXT_RUN_MINUTES
    ...
    global TRAILING_ATR_MULT, UNIVERSE_REFRESH_INTERVAL_MINUTES
    global PAIR_LIST_FROM_ENV, PAIR_UNIVERSE_FROM_ENV, INDICATOR_POOL_FROM_ENV, TIMEFRAMES_FROM_ENV

    raw_pairs = (os.getenv("PAIR_LIST") or "").strip()
    PAIR_LIST_FROM_ENV = bool(raw_pairs)
    if raw_pairs:
        PAIR_LIST = [p.strip() for p in raw_pairs.split(",") if p.strip()]
    else:
        PAIR_LIST = list(DEFAULT_PAIR_CANDIDATES)
    ...
    trailing_raw = os.getenv("TRAILING_ATR_MULT", os.getenv("TRAILING_ATR"))
    try:
        TRAILING_ATR_MULT = max(0.0, float(trailing_raw)) if trailing_raw not in (None, "") else 0.0
    except (TypeError, ValueError):
        TRAILING_ATR_MULT = 0.0
    try:
        UNIVERSE_REFRESH_INTERVAL_MINUTES = max(30.0, float(env_int("AI_UNIVERSE_REFRESH_MINUTES", 360)))
    except (TypeError, ValueError):
        UNIVERSE_REFRESH_INTERVAL_MINUTES = 360.0
    ...
    default_next = safe_float(os.getenv("DEFAULT_NEXT_RUN_MINUTES"))
    DEFAULT_NEXT_RUN_MINUTES = default_next if (default_next and default_next > 0) else 30.0

    universe_env = (os.getenv("PAIR_UNIVERSE") or os.getenv("PAIR_POOL") or os.getenv("PAIR_LIST_UNIVERSE") or "").strip()
    PAIR_UNIVERSE_FROM_ENV = bool(universe_env)
    if universe_env:
        PAIR_UNIVERSE = [p.strip() for p in universe_env.split(",") if p.strip()]
    else:
        PAIR_UNIVERSE = list(PAIR_LIST) if PAIR_LIST else list(DEFAULT_PAIR_CANDIDATES)
    if not PAIR_UNIVERSE:
        PAIR_UNIVERSE = list(DEFAULT_PAIR_CANDIDATES)

    indicator_env = (os.getenv("INDICATOR_POOL") or os.getenv("AI_INDICATOR_POOL") or "").strip()
    INDICATOR_POOL_FROM_ENV = bool(indicator_env)
    INDICATOR_POOL = [s.strip().lower() for s in indicator_env.split(",") if s.strip()]
    if not INDICATOR_POOL:
        INDICATOR_POOL = list(DEFAULT_INDICATOR_CANDIDATES)

    timeframe_env = (os.getenv("DEFAULT_TIMEFRAMES") or os.getenv("TIMEFRAME_POOL") or os.getenv("AI_TIMEFRAMES") or "").strip()
    TIMEFRAMES_FROM_ENV = bool(timeframe_env)
    DEFAULT_TIMEFRAMES = [tf.strip() for tf in timeframe_env.split(",") if tf.strip()]
    if not DEFAULT_TIMEFRAMES:
        DEFAULT_TIMEFRAMES = list(DEFAULT_TIMEFRAME_CANDIDATES)
    ...
    NEWS_API_ENDPOINT = (os.getenv("CRYPTO_NEWS_ENDPOINT") or "")
    ...
    _apply_cached_universe_settings()
    if not PAIR_UNIVERSE:
        PAIR_UNIVERSE = list(PAIR_LIST) if PAIR_LIST else list(DEFAULT_PAIR_CANDIDATES)
    if not INDICATOR_POOL:
        INDICATOR_POOL = list(DEFAULT_INDICATOR_CANDIDATES)
    ...
    if LOG_TIMEZONE and parsed_tz is None:
        if not _LOG_TZ_WARNING_EMITTED:
            print(f"[WARN] LOG_TIMEZONE '{LOG_TIMEZONE}' �� ���������, ������������ ��������� �����.")
...
def get_higher_tf(exchange, symbol, tf="4h", limit=120):
    try:
        ...
        log(f"?? �� ������� �������� higher_tf {tf}: {e}", Fore.YELLOW)
        return []
    ...
    try:
        if hasattr(exchange,"fetchFundingRate"):
            ...
            if funding_rate is not None:
                summary_parts.append(f"������� ������ {funding_rate*100:.4f}%")
            if next_time:
                summary_parts.append(f"����. ������� {next_time}")
        ...
        if hasattr(exchange, "fetchFundingRateHistory"):
            ...
            if history:
                ...
                summary = (
                    f"��������� ������ {rate*100:.4f}% �� {ts_iso}"
                    if rate is not None and ts_iso else
                    f"��������� ������ {rate*100:.4f}%"
                    if rate is not None else
                    f"� ������� ������� ������ �� {ts_iso}" if ts_iso else "������� ��������"
                )
                return {
        ...
        log(f"?? Funding rate ����������: {e}", Fore.YELLOW)
    return {}
...
def get_open_interest(exchange, symbol):
    try:
        ...
        log(f"?? Open interest ����������: {e}", Fore.YELLOW)
    return []


def get_news_from_rss(base_symbol: str, limit: int):
    if not RSS_FEEDS:
        return {"summary": "RSS ��������� �� ���������", "items": []}
    if feedparser is None:
        return {"summary": "feedparser �� ����������", "items": []}
    ...
    if symbol_articles:
        ...
        summary = f"RSS {len(selected)} ������� �� {base_upper}"
    else:
        selected = general_articles[:limit]
        summary = (
            f"RSS {len(selected)} ����� ��������" if selected else "������� �� ������� (RSS)"
        )
    ...
    if NEWS_API_TOKEN:
        ...
        try:
            ...
            if news_items:
                ...
                summary = f"{len(news_items)} �������� API, ��������� {latest}"
                return {"summary": summary, "items": news_items, "asset": base, "source": "cryptopanic"}
            log(f"?? CryptoPanic �� ������ ������� ��� {symbol}, ���������� RSS", Fore.LIGHTBLACK_EX)
            ...
            log(f"?? CryptoPanic ���������� ��� {symbol}: {status_text} � {e}", color)
        except Exception as e:
            log(f"?? CryptoPanic ���������� ��� {symbol}: {e}", Fore.YELLOW)
    # Fallback to RSS
...
def ensure_position_mode(exchange):
    ...
    try:
        if hasattr(exchange, "set_position_mode") and PAIR_LIST:
            ...
            log(f"?? ����� ������� ����������: {desired}", Fore.LIGHTBLACK_EX)
    except Exception as e:
        code = get_bybit_retcode(e)
        if code == 110025:
            log(f"?? ����� ������� ��� ���������� ({desired}, ��� {code})", Fore.LIGHTBLACK_EX)
        else:
            log(f"?? �� ������� ���������� ����� ������� ({desired}): {e}", Fore.YELLOW)
...
def _wallet_and_pnl(usdt_info: dict, equity: float) -> tuple[float, float]:
    wallet_sources = (
        "walletBalance",
        "cashBalance",
        "accountBalance",
    ...
    wallet_val = None
    for key in wallet_sources:
        wallet_val = safe_float(usdt_info.get(key))
        if wallet_val is not None:
            break
    if wallet_val is None:
        wallet_val = safe_float(usdt_info.get("totalInitialMargin"))
    if wallet_val is None:
        return 0.0, 0.0
    pnl = equity - wallet_val
    return wallet_val, pnl


def compute_order_amount(order, current_position, fallback_amount=None):
        ...
        try:
            ...
            if current_position:
                try:
                    ...
                    base = None
            if (base is None or base <= 0) and fallback_amount not in (None, "", 0):
                try:
                    base = abs(float(fallback_amount))
                except (TypeError, ValueError):
                    base = None
...
def execute_extra_orders(
    ...
    preserve_reduce_only: bool = False
):
    executed = []
    ...
    if not isinstance(orders, (list, tuple)):
        log(f"?? ������������ ������ orders ��� {symbol}: ��������� ������", Fore.YELLOW)
        return executed, False
    cancelled_success = []
    cancel_errors = []
    for idx, order in enumerate(orders, 1):
        if not isinstance(order, dict):
            log(f"?? ������� order #{idx} ��� {symbol}: ��������� ������", Fore.YELLOW)
            continue
        ...
        amount = compute_order_amount(order, current_position, fallback_amount)
        ...
        if not side:
            log(f"?? �� ������ side � order #{idx} ��� {symbol}", Fore.YELLOW)
            continue
        if amount is None:
            log(f"?? �� ������� ���������� ����� ������ #{idx} ��� {symbol}", Fore.YELLOW)
            continue
        ...
        if ccxt_type in ("limit", "stopLimit", "takeProfit", "stopLoss") and price is None:
            log(f"?? ����� ���� ��� ������ #{idx} ({ccxt_type}) {symbol}", Fore.YELLOW)
            continue

        if params.get("reduceOnly") and not preserve_reduce_only:
            ...
            if existing_list:
                for existing_order in existing_list:
                    ...
                    if success:
                        ...
                        log(f"??? ������ ������������ reduce-only ����� {oid} ��� {symbol} ����� �������", Fore.LIGHTBLUE_EX)
                    else:
                        cancel_errors.append((oid, err))
                        log(f"?? �� ������� �������� reduce-only ����� {oid} ��� {symbol}: {err}", Fore.YELLOW)
        ...
        if params.get("reduceOnly"):
            if not current_position or abs(position_amount) == 0:
                log(f"?? ������� reduce-only ������ #{idx} ��� {symbol}: ������� �����������", Fore.YELLOW)
                continue
            amount = min(amount, abs(position_amount))
            if amount <= 0:
                log(f"?? ������� reduce-only ������ #{idx} ��� {symbol}: ����� = 0", Fore.YELLOW)
                continue
        ...
        if amount_min and amount < amount_min:
            if params.get("reduceOnly") and abs(position_amount) >= amount_min:
                ...
                log(f"?? ����� #{idx} ��� {symbol}: ����� {amount} < ������������ {amount_min}", Fore.YELLOW)
                continue
        ...
        if amount_precision is not None:
            try:
                ...
                log(f"?? ����� #{idx} ��� {symbol}: ������ ���������� ������: {exc}", Fore.YELLOW)
                continue
        if amount <= 0:
            log(f"?? ������� ������ #{idx} ��� {symbol}: �������� ����� = 0", Fore.YELLOW)
            continue
        ...
        if price is not None:
            if price_precision is not None:
                try:
                    ...
                    log(f"?? ������������ ���� {price} � ������ #{idx} ��� {symbol}: {exc}", Fore.YELLOW)
                    continue
            if min_cost and amount * price < min_cost:
                log(f"?? ����� #{idx} ��� {symbol}: notional {amount * price:.4f} < min cost {min_cost}", Fore.YELLOW)
                continue
        ...
        try:
            ...
            log(f"??? ���. ����� ��� {symbol}: {desc}", Fore.LIGHTBLUE_EX)
        except Exception as e:
            log(f"? ������ ���. ������ #{idx} ��� {symbol}: {e}", Fore.RED)
    if cancelled_success:
        send_trade_update(f"??? {symbol}: �������� ������ {', '.join(cancelled_success)} ����� �������")
    if cancel_errors:
        errs = []
        for oid, err in cancel_errors:
            if "110001" in str(err):
                log(f"?? {symbol}: ����� {oid} ��� ����������� ��� ������ (110001).", Fore.LIGHTBLACK_EX)
                continue
            errs.append(f"{oid}: {err}")
        if errs:
            send_tg(f"?? {symbol}: ������ ������ ������� � {'; '.join(errs)}")
...
def ensure_position_protection(exchange, symbol, position, df_primary, open_orders, config=None):
    ...
    df_calc = None
    if isinstance(df_primary, pd.DataFrame) and not df_primary.empty:
        df_calc = df_primary.copy()
    if df_calc is None:
        return open_orders
    if "atr" not in df_calc.columns:
        try:
            ...
            log(f"?? {symbol}: �� ������� ��������� ATR ��� ������ ������� ({exc})", Fore.YELLOW)
            return open_orders
    ...
    if not has_stop and math.isfinite(stop_price):
        stop_direction = 1 if stop_price >= price else 2
        stop_params = {
            "reduceOnly": True,
            ...
            "stopLoss": stop_price,
            "triggerBy": "LastPrice",
            "triggerDirection": stop_direction,
        }
        if position_idx is not None:
            stop_params["positionIdx"] = position_idx
    ...
    if not has_take and math.isfinite(take_price):
        take_params = {"reduceOnly": True}
        if position_idx is not None:
            take_params["positionIdx"] = position_idx
        protection_orders.append({
            "type": "limit",
            "side": protection_side,
            "price": take_price,
    ...
    if trailing_mult > 0 and not has_trailing:
        trailing_distance = trailing_mult * atrv
        if math.isfinite(trailing_distance) and trailing_distance > 0:
            trailing_direction = 1 if stop_price >= price else 2
            trailing_params = {
                "reduceOnly": True,
                "trailingStop": trailing_distance,
                "triggerBy": "LastPrice",
                "triggerDirection": trailing_direction,
            }
            if position_idx is not None:
    ...
    executed, actions = execute_extra_orders(
        exchange,
        symbol,
        protection_orders,
        ...
        fallback_amount=qty,
        preserve_reduce_only=True
    )
    if executed:
        send_trade_update(f"??? {symbol}: ����-������ �������\n- " + "\n- ".join(executed))
    if actions:
        try:
            open_orders = fetch_open_orders_for_symbol(exchange, symbol)
        except Exception as exc:
            log(f"?? �� ������� �������� ������ �������� ������� ��� {symbol}: {exc}", Fore.YELLOW)
    return open_orders
def ai_decision(
    symbol,
    df_primary,
...
(2) ���� 30m ���������� ������������� �������� ������ ������� ������ �� 4h � ����������� ���������� � �������� �����; 
(3) skip ������������ ������ ��� �������� ��������� �������� ��� ����� ���������������. 
����������� ���������� EMA20/EMA50, RSI(14), ATR(14) �� 30m � 4h, �������� �������� ����/����. 
���� ����������� < 70% ��� ������� ���������� � ������� ������� �������������� ������ ����� ���� 'needs' 
(��������: higher_tf:<tf>, funding, open_interest, news, � ����� {"timeframes":["1h"],"indicators":[{"indicator":"ema","length":55}, "atr14"]}), � ������ ����� ���. �������� ������� �������� ��������. 
���� ������� ��� �������, �� �������� � ������: ����� ������������� ���������� ����������, �������� ��� ���������. 
���� �� ������� ���� �������� ��������/����-������ (open_orders), �� �������� �� ��� ����������. 
��� ������/������ ������� ��������� cancel_orders � replace_orders. 
��� ��������� ��������, �������������� �������/������, ���������� � ������ �������� ��������� ������ 'orders', 
�������� ������ � ����� CCXT (type, side, amount/percent, price, params). 
���� ��������� action="skip", ����������� ����� �������, �������� �� ��������� ���� �����������. 
����� ������ � ������� JSON ��� ������."""
    # >>>>>>>>>>>> ����� ����������� <<<<<<<<<<<<
    ...
    def build_prompt(extra=None, bias=False):
        news_desc = "CryptoPanic API (fallback: RSS ������-�����)" if NEWS_API_TOKEN else "RSS ������� �� ��������� ������"
        prompt = {
            "������": symbol,
            "�������": {
                "equity_total": equity,
            ...
            "�������": equity,
            "����������": current_context,
            "�������_�������": position_payload or {"������": "��� �������"},
            "��������_������": open_orders,
            "���_���������_������": [
                "��������� ������ 'orders', ���� ����� ��������� �������������� ������.",
                "������: orders=[{\"type\":\"limit\",\"side\":\"sell\",\"amount\":0.001,\"price\":111500,\"reduceOnly\":true,\"note\":\"��������� ����\"}].",
                "��� ���������� �������� ����� ������� amountPercent ������ amount; trailing_stop ��������� ����� params (��������, {'trailingStop':50}).",
                "�������� open_orders (��. ���� 'open_orders'): ������� ������������ ������������ �������/������.",
                "��� ���������� ������/������ ��������� cancel_orders ��� replace_orders (������� ����� id ������, ����� ����� ����� �����)."
            ],
            "��������": {
                "��������": [
                    "1) ������� ������ 30m/4h � RSI.",
                    "2) ���� ����������� <70% ��� ������� ���������� � ������� needs (higher_tf:<tf>, funding, open_interest, news).",
                    "3) ����� ��������� �������������� ������ ������ ������������� ��������."
                ],
                "�������_�����": {
                    "�������������_��������": "open" if ema_trend_bias else "skip",
                    "�������": ema_trend_bias,
                    "��������": (
                        f"������������ ����� {ema_trend_label} �� EMA20/EMA50 �� 30m."
                        f" �� 4h ����� {higher_trend_label}. ��� �� ���������� ������� ������������ �����."
                    ),
                    "������_ema20": ema_slope
                },
                "����������": {
    ...
    def prepare_messages(stage="initial", extra=None, bias=False):
        ...
        while True:
            ...
            attempts += 1
            if attempts > 50:
                log(f"?? ������� ��������� �� ������������ � ����� ({stage}) ��� {symbol}", Fore.YELLOW)
                break
        ...
        if trimmed:
            ...
            trim_text = (
                f"?? �������� ������� ({reasons_text}) �� "
                f"{context_counts['30m']}?30m � {context_counts['4h']}?4h "
                f"��-�� ������ ({tokens} �������, ����: {stage}) ��� {symbol}"
            )
            log(trim_text, Fore.MAGENTA)
            send_tg(trim_text)
        if hard_limit and tokens > hard_limit:
            log(f"?? ����� ������� �������� ���� ����� ������� ({tokens}>{TOKEN_LIMIT}, ����: {stage}) ��� {symbol}", Fore.YELLOW)
    ...
    def ensure_skip_reason(decision_obj):
        ...
        if df_30m.empty:
            ...
            if pd.notna(ema20_val) and pd.notna(ema50_val):
                relation = "����" if ema20_val < ema50_val else "����"
                parts.append(f"EMA20 {ema20_val:.2f} {relation} EMA50 {ema50_val:.2f}")
            elif pd.notna(ema20_val) or pd.notna(ema50_val):
                parts.append(f"EMA ������ �������� (EMA20={ema20_val}, EMA50={ema50_val})")
            if pd.notna(rsi_val):
                parts.append(f"RSI14 {rsi_val:.1f}")
            if pd.notna(atr_val):
                parts.append(f"ATR14 {atr_val:.2f}")
            details = "; ".join(parts) if parts else "��� �������� �������� EMA/RSI/ATR"
        decision_obj["reason"] = (reason + " � " if reason else "") + f"����������: {details}"
        log(f"?? ������� skip ��������� ������������ ��� {symbol}", Fore.LIGHTBLACK_EX)
        return decision_obj

    messages_init, tokens_init, _ = prepare_messages(stage="initial")
    log(f"?? ������ ������� (initial) ��� {symbol}: {tokens_init}", Fore.LIGHTBLACK_EX)
    ...
    log(f"?? OpenAI initial ������ ��� {symbol}: {duration_init:.2f} c", Fore.LIGHTBLACK_EX)
    ...
    if needs:
        if auto_needs_triggered:
            log(f"?? ���������� ��������������� ��������� �� ������� ������ �����������: {needs}", Fore.CYAN)
            send_tg(f"?? ���������� ������ ��� {symbol}: {needs}")
        else:
            log(f"?? ������ ��������� �������������� ��������: {needs}", Fore.CYAN)
            send_tg(f"?? ������ ��������� �������� ��� {symbol}: {needs}")
        extra = {}
        ...
        for key,val in extra.items():
            if isinstance(val, list):
                stats_report.append(f"{key}: {len(val)} �������")
                ...
                if key == "higher_tf":
                    ...
                    for tf_name, payload in (val.get("timeframes") or {}).items():
                        ...
                        tf_entries.append(
                            f"{tf_name}: ���������� {', '.join(indicators.keys()) or '���'}"
                        )
                    if tf_entries:
        ...
        try:
            ...
            log(f"⚠️ OpenAI extra timeout for {symbol}: {exc} ({duration_extra:.2f}s)", Fore.YELLOW)
            send_tg(f"⚠️ OpenAI extra timeout for {symbol}: {exc}")
            decision = {"symbol": sym, "action": "skip", "reason": "OpenAI extra timeout"}
            decision = ensure_skip_reason(decision)
            return decision
        duration_extra = time.perf_counter() - start_extra
        log(f"?? OpenAI extra ��?��?��?��?��?��? ��?��?��? {symbol}: {duration_extra:.2f} c", Fore.LIGHTBLACK_EX)
        msg2 = res2.choices[0].message.content
        decision = json.loads(msg2)
        needs_followup = decision.get("needs", [])
        if needs_followup:
            log(f"?? ����� ������������ ������ ��� ��� ����������� {needs_followup} ��� {symbol}", Fore.LIGHTBLACK_EX)
...
def run_cycle():
    ...
    refresh_settings()
    log(
        f"⚙️ Settings: LEVERAGE={LEVERAGE} RISK_PCT={RISK_PCT} SL_ATR={SL_ATR} TP_ATR={TP_ATR} MAX_OPEN_POSITIONS={MAX_OPEN_POSITIONS}",
        Fore.LIGHTBLACK_EX
    )
    ex = init_exchange()
    ex.load_markets()
    ...
    if MAX_OPEN_POSITIONS > 0 and open_positions is None:
        log("?? �� ������� ���������� ���������� �������� ������� � ����� �� �������� �������� �� ���� ����", Fore.YELLOW)
        open_positions = None
    equity, available_margin, balance_snapshot = fetch_usdt_equity(ex)
    if equity <= 0:
        equity = 64.0
    ...
    usdt_info_start = (balance_snapshot.get("USDT") or balance_snapshot.get("USDT:USDT") or {})
    wallet_start, pnl_start = _wallet_and_pnl(usdt_info_start, equity)
    start_balance_text = (
        f"Equity: {equity:.2f} USDT | Free: {available_margin:.2f} USDT | "
        f"PnL: {pnl_start:.2f} USDT"
    )
    version_label = f'v{BOT_VERSION}'
    if not IS_LATEST_VERSION:
        version_label += f' (latest {EXPECTED_VERSION})'
    session_banner = f"{'=' * 18} SESSION START {session_start} {version_label} {'=' * 18}"
    log(session_banner, Fore.MAGENTA)
    if not CHANGELOG_BROADCASTED and CHANGELOG_TEXT:
        for line in CHANGELOG_TEXT.splitlines():
            stripped = line.strip()
            if stripped:
                log(f"Changelog: {stripped}", Fore.MAGENTA)
    if not IS_LATEST_VERSION:
        fallback_notice = f"?? Running fallback build {version_label}."
        log(fallback_notice, Fore.YELLOW)
        send_tg(fallback_notice)
    log(f"?? Bot {version_label} launched. {start_balance_text}", Fore.GREEN)
    send_tg(f"=========== START SESSION {session_start} ===========")
    send_tg(f"?? Bot {version_label} launched. {start_balance_text}")
    ...
    news_cache = {}
    target_map = {}
    symbol_config_map: dict[str, dict[str, float]] = {}
    selected_symbols = []
    ...
    if selection:
        ...
        for target in selection.get("targets") or []:
            ...
            target_map[sym_sel] = target_entry
            sanitized = sanitize_symbol_overrides(
                target.get("config")
                or target.get("settings")
                or target.get("symbol_config")
            )
            if sanitized:
                symbol_config_map[sym_sel] = sanitized
                target_entry["symbol_config"] = sanitized
                target_entry["config"] = sanitized
            selected_symbols.append(sym_sel)
        selection_reason = selection.get("reason")
        if selection_reason:
            log(f"?? ����������� �����: {selection_reason}", Fore.CYAN)
            send_tg(f"?? ��������: {selection_reason}")
        confidence = selection.get("confidence")
        if confidence:
            log(f"?? ����������� ������: {confidence}", Fore.LIGHTBLACK_EX)
        ...
        for key in ("symbol_configs", "symbol_settings", "per_symbol_config"):
            sanitized_bundle = sanitize_symbol_config_map(selection.get(key))
            if sanitized_bundle:
                for sym_key, overrides in sanitized_bundle.items():
                    merged = dict(symbol_config_map.get(sym_key, {}))
                    merged.update(overrides)
                    symbol_config_map[sym_key] = merged
        if symbol_config_map:
            for sym_key, overrides in symbol_config_map.items():
                entry = target_map.get(sym_key)
                if entry is not None:
                    entry["symbol_config"] = overrides
                    entry["config"] = overrides
            summary_lines = []
            for sym_key, overrides in symbol_config_map.items():
                parts = [f"{k}={overrides[k]}" for k in sorted(overrides)]
                summary_lines.append(f"{sym_key}: {', '.join(parts)}")
            if summary_lines:
                for chunk_start in range(0, len(summary_lines), 3):
                    chunk = summary_lines[chunk_start:chunk_start + 3]
                    text = "; ".join(chunk)
                    log(f"Per-symbol settings: {text}", Fore.LIGHTBLUE_EX)
                    send_tg(f"Per-symbol settings: {text}")
    apply_config_overrides_from_selection(selection)
    forced_symbols = set()
    ...
    for i,sym in enumerate(symbols_sequence,1):
        ...
        try:
            ...
            base_symbol_config = symbol_config_map.get(sym, {})
            if base_symbol_config and "symbol_config" not in symbol_meta:
                symbol_meta["symbol_config"] = dict(base_symbol_config)
            if base_symbol_config and "config" not in symbol_meta:
                symbol_meta["config"] = dict(base_symbol_config)
            requested_timeframes = list(dict.fromkeys(
                list(global_timeframes or []) + list(symbol_meta.get("timeframes") or [])
            ))
            if not requested_timeframes:
                log(f'?? {sym}: ������ �� ������� ����������, ���������� fallback {TIMEFRAME}', Fore.YELLOW)
            ...
            for tf in requested_timeframes:
                try:
                    ...
                    log(f"?? �� ������� �������� {tf} ��� {sym}: {exc_fetch}", Fore.YELLOW)
                    continue
            ...
            if primary_tf not in timeframe_dfs:
                try:
                    ...
                    log(f"?? �� ������� �������� ������� ��������� {primary_tf} ��� {sym}: {exc_fetch}", Fore.RED)
                    continue
            ...
            if base_symbol_config:
                extra_serialized["symbol_config"] = base_symbol_config
            ...
            if not news_payload_symbol:
                try:
                    ...
                    log(f"?? �� ������� �������� ������� ��� {sym}: {news_exc}", Fore.YELLOW)
            ...
            if not dec:
                continue
            decision_overrides = sanitize_symbol_overrides(
                dec.get("config") or dec.get("settings") or dec.get("symbol_config")
            )
            effective_config = dict(base_symbol_config) if base_symbol_config else {}
            if decision_overrides:
                effective_config.update(decision_overrides)
            cfg_leverage = effective_config.get("leverage", LEVERAGE)
            cfg_risk_pct = effective_config.get("risk_pct", RISK_PCT)
            cfg_sl_atr = effective_config.get("sl_atr", SL_ATR)
            cfg_tp_atr = effective_config.get("tp_atr", TP_ATR)
            cfg_trailing_atr = effective_config.get("trailing_atr_mult", TRAILING_ATR_MULT)
            cfg_order_util = effective_config.get("order_margin_utilization", ORDER_MARGIN_UTILIZATION)
            if decision_overrides:
                override_summary = ", ".join(f"{k}={v}" for k, v in decision_overrides.items())
                log(f"Symbol overrides for {sym}: {override_summary}", Fore.LIGHTBLUE_EX)
                send_tg(f"Symbol overrides for {sym}: {override_summary}")
            ...
            if base_symbol_config:
                log_payload["base_config"] = dict(base_symbol_config)
            if decision_overrides:
                log_payload["decision_config"] = decision_overrides
            if effective_config:
                log_payload["effective_config"] = effective_config
            save_json_line(AI_LOG_FILE, log_payload)
            ...
            def try_cancel(order_id: str, source: str):
                ...
                if success:
                    cancelled_ids.add(oid)
                    cancelled_success.append((oid, source))
                    log(f"??? ������ ����� {oid} ��� {sym} (�������� {source})", Fore.LIGHTBLUE_EX)
                else:
                    cancel_failures.append((oid, err))
                    log(f"?? �� ������� �������� ����� {oid} ��� {sym}: {err}", Fore.YELLOW)
            ...
            if cancelled_success:
                ...
                send_trade_update(f"??? �������� ������ �� {sym}: {summary}")
                # ��������� ������ �������� ������� ����� ������
                open_orders_symbol = fetch_open_orders_for_symbol(ex, sym)
            if cancel_failures:
                errors = "; ".join(f"{oid}: {err}" for oid, err in cancel_failures)
                send_tg(f"?? �� ������� �������� ������ �� {sym}: {errors}")

            if action == "skip":
                log(f"? ������� {sym} ({reason})", Fore.WHITE)
                send_tg(f"? ������� {sym} � {reason or '������� �� �������'}")
            elif action == "close":
                if not current_position or abs(float(current_position.get("amount") or 0)) == 0:
                    log(f"?? ������� �� {sym} �����������, ������ ��������� ({reason})", Fore.YELLOW)
                    send_tg(f"?? {sym}: �������� ��������� � ��� �������� �������")
                else:
                    ...
                    if qty == 0:
                        ...
                        try:
                            ...
                            log(f"?? ������� ������� {sym} ({reason})", Fore.YELLOW)
                            send_trade_update(f"?? ������ {sym} {close_side.upper()} {qty:.4f} � {reason or '������� �� �������'}")
                            positions_map, open_positions = fetch_positions_snapshot(ex, symbols_filter=PAIR_UNIVERSE or None)
                            current_position = positions_map.get(sym)
                        except Exception as e:
                            err_text = str(e)
                            log(f"? ������ �������� {sym}: {err_text}", Fore.RED)
                            send_tg(f"? ������ �������� ��� {sym}: {err_text}")
            elif action == "hold":
                log(f"? ���������� {sym} ({reason})", Fore.BLUE)
                send_tg(f"? {sym}: ���������� ������� � {reason or '������� �� �������'}")
            elif action == "open":
                primary_order_sent = False
                if current_position and abs(float(current_position.get("amount") or 0)) > 0:
                    log(f"?? ������� �� {sym} ��� ������� (side={current_position.get('side')}, amount={current_position.get('amount')}), ���������� ��������� ��������", Fore.YELLOW)
                    send_tg(f"?? {sym}: ������� ��� �������, ������ open ��������")
                elif MAX_OPEN_POSITIONS > 0 and open_positions is not None and open_positions >= MAX_OPEN_POSITIONS:
                    log(f"? ����� �������� ������� ��������� ({open_positions}/{MAX_OPEN_POSITIONS}), ���������� {sym}", Fore.YELLOW)
                    send_tg(f"? ����� �������� ������� ��������� ({open_positions}/{MAX_OPEN_POSITIONS}), {sym} ��������")
                    allow_extra_orders = False
                    planned_entry_qty = None
                    continue
                else:
                    log(f"?? ������ {side.upper()} ({reason})", Fore.GREEN)
                    send_trade_update(f"?? {sym} {side.upper()} � {reason or '������� �� �������'}")
                    if df.empty:
                        log(f"?? ��� ������ 30m ��� {sym}, ���������� ��������", Fore.YELLOW)
                        send_tg(f"?? {sym}: ������������ ������ ��� �������� �������")
                        continue
                    df["atr"] = atr(df,14)
                    last_row = df.iloc[-1]
                    ...
                    if not (math.isfinite(price) and math.isfinite(atrv) and atrv > 0):
                        log(f"?? �� ������� ���������� ATR/���� ��� {sym}, ������� �������", Fore.YELLOW)
                        send_tg(f"?? {sym}: ��� �������� �������� ATR ��� ������� �������")
                        continue
                    sl = price - cfg_sl_atr * atrv if side == "buy" else price + cfg_sl_atr * atrv
                    tp = price + cfg_tp_atr * atrv if side == "buy" else price - cfg_tp_atr * atrv
                    ...
                    if duplicate_order:
                        ...
                        log(f"?? ������� ��������� ������ {sym}: ��� ��������� {side.upper()} @ {dup_price}", Fore.LIGHTBLACK_EX)
                        send_tg(f"?? {sym}: ����� {side.upper()} @ {dup_price} ��� �������, ����� ����� �� ��������")
                        continue
                    risk_distance = abs(price - sl)
                    if risk_distance <= 0 or not math.isfinite(risk_distance):
                        log(f"?? ���������� ��������� �� SL ��� {sym}, ������� �������", Fore.YELLOW)
                        send_tg(f"?? {sym}: �� ������� ��������� ���������� �� �����")
                        continue
                    risk_budget_base = max(0.0, min(equity, available_margin))
                    risk_capital = risk_budget_base * cfg_risk_pct
                    if risk_capital <= 0:
                    ...
                    if notional < MIN_NOTIONAL_USDT:
                        ...
                        notional = qty * price
                    effective_margin = max(0.0, available_margin * cfg_order_util)
                    max_notional = effective_margin * cfg_leverage
                    if max_notional <= 0:
                        log(f"? ��������� ����� ��� {sym} ���������", Fore.YELLOW)
                        send_tg(f"? {sym}: ��������� ����� ���������")
                        continue
                    ...
                    if notional > max_notional:
                        ...
                        log(f"?? ����� {sym} �������� �� {qty:.4f} (~{notional:.2f} USDT) ��-�� ������ �����", Fore.LIGHTBLACK_EX)
                    try:
                        qty = float(ex.amount_to_precision(sym, qty))
                    except Exception:
                        qty = float(round(qty, 8))
                    if qty <= 0:
                        log(f"?? ����� ���������� ����� ���� ? 0 ��� {sym}", Fore.YELLOW)
                        continue
                    notional = qty * price
                    if notional < MIN_NOTIONAL_USDT:
                        log(f"? ����� ���������� ����� {sym} ���� ������������ ({notional:.2f} USDT)", Fore.YELLOW)
                        send_tg(f"? {sym}: ����� ����� ���������� ���� �������� ({notional:.2f} USDT)")
                        continue
                    planned_entry_qty = qty
                    try:
                        ex.set_leverage(cfg_leverage, sym)
                    except Exception as e:
                        planned_entry_qty = None
                        code = get_bybit_retcode(e)
                        if code == 110043:
                            log(f"?? ����� {LEVERAGE}x ��� ����������� ��� {sym} (��� {code})", Fore.LIGHTBLACK_EX)
                        else:
                            log(f"?? ������ ��������� �����: {e}", Fore.YELLOW)
                    try:
                        ...
                        if position_idx is not None:
                            params["positionIdx"] = position_idx
                        margin_required = notional / cfg_leverage if cfg_leverage else notional
                        ex.create_order(sym, "limit", side, qty, price, params)
                        log(f"? ����� {sym} {side.upper()} {qty:.4f}@{price:.2f} SL:{sl:.2f} TP:{tp:.2f}", Fore.GREEN)
                        send_tg(
                            f"? {sym} {side.upper()} @ {price:.2f}\n"
                            f"SL {sl:.2f} TP {tp:.2f}\n"
                            f"����� {notional:.2f} USDT, ����� {margin_required:.2f} USDT"
                        )
                        positions_map, open_positions = fetch_positions_snapshot(ex, symbols_filter=PAIR_UNIVERSE or None)
                        current_position = positions_map.get(sym)
                        primary_order_sent = True
                    except Exception as e:
                        err_text = str(e)
                        planned_entry_qty = None
                        log(f"? ������ ������: {err_text}", Fore.RED)
                        send_tg(f"? ������ ������ ��� {sym}: {err_text}")
                    if not primary_order_sent and (not current_position or abs(float(current_position.get("amount") or 0)) == 0):
                        allow_extra_orders = False
            else:
                if action not in ("hold", "manage", "none", "", None):
                    log(f"?? ����������� �������� \"{action}\" ��� {sym}, ��������� ������ �������������� �������", Fore.YELLOW)

            if extra_orders and allow_extra_orders:
                executed, actions_performed = execute_extra_orders(
                    ex,
                    sym,
                    extra_orders,
                    current_position=current_position,
                    open_orders=open_orders_symbol,
                    fallback_amount=planned_entry_qty
                )
                if executed:
                    send_trade_update("??? " + sym + " ���. ������:\n- " + "\n- ".join(executed))
                if actions_performed:
                    positions_map, open_positions = fetch_positions_snapshot(ex, symbols_filter=PAIR_UNIVERSE or None)
                    current_position = positions_map.get(sym)
                    open_orders_symbol = fetch_open_orders_for_symbol(ex, sym)
            ...
            if current_position and abs(float(current_position.get("amount") or 0)) > 0:
                try:
                    open_orders_symbol = ensure_position_protection(
                        ex,
                        sym,
                        current_position,
                        df,
                        open_orders_symbol,
                        config={"sl_atr": cfg_sl_atr, "tp_atr": cfg_tp_atr, "trailing_atr_mult": cfg_trailing_atr}
                    )
                except Exception as protect_exc:
                    log(f"?? {sym}: �� ������� ��������� �������� ������ � {protect_exc}", Fore.YELLOW)
    ...
    if selection_next_run is not None:
        ...
        if minutes_val and minutes_val > 0:
            ...
            next_dt = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=minutes_val)
            next_local = next_dt.astimezone(LOG_TZINFO or datetime.datetime.now().astimezone().tzinfo)
            next_msg = (
                f"?? ��������� ������ ����� {minutes_val:.0f} ��� (~{next_local.strftime('%Y-%m-%d %H:%M:%S %Z')})"
            )
            log(next_msg, Fore.CYAN)
            send_tg(next_msg)
        else:
            log("?? ������ ������� ������������ next_run_minutes.", Fore.YELLOW)
    elif selection_next_time:
        parsed_time = selection_next_time.strip() if isinstance(selection_next_time, str) else ""
        if parsed_time:
            iso_candidate = parsed_time.replace('Z', '+00:00')
            try:
                ...
                delta = (target_dt - utc_now).total_seconds() / 60.0
                if delta > 0:
                    next_delay_minutes = delta
                    next_local = target_dt.astimezone(LOG_TZINFO or datetime.datetime.now().astimezone().tzinfo)
                    msg = f"?? ��������� ������ �� ������: {next_local.strftime('%Y-%m-%d %H:%M:%S %Z')}"
                    log(msg, Fore.CYAN)
                    send_tg(msg)
                else:
                    log("?? ������ ������� ��������� ����� next_run_time.", Fore.YELLOW)
            except Exception as exc:
                log(f"?? �� ������� ��������� next_run_time '{selection_next_time}': {exc}", Fore.YELLOW)
    if next_delay_minutes is None:
        next_delay_minutes = DEFAULT_NEXT_RUN_MINUTES
        fallback_msg = f"?? ���������� �������� �� ��������� {next_delay_minutes:.0f} ���."
    ...
    if decisions_total>0:
        ...
        summary=f"?? ����������: ������� {counts['open']} ({pct['open']:.1f}%), " \
                f"������� {counts['close']} ({pct['close']:.1f}%), " \
                f"������� {counts['skip']} ({pct['skip']:.1f}%) � ����� {decisions_total}"
        log(summary, Fore.CYAN)
        send_tg(summary)
    send_tg("? ���� ��������.")
    equity_end, available_end, balance_end = fetch_usdt_equity(ex)
    end_dt = _current_log_time()
    end_stamp = end_dt.strftime("%Y-%m-%d %H:%M:%S %Z")
    usdt_info_end = (balance_end.get("USDT") or balance_end.get("USDT:USDT") or {})
    wallet_end, pnl_end = _wallet_and_pnl(usdt_info_end, equity_end)
    end_balance_text = (
        f"Equity: {equity_end:.2f} USDT | Free: {available_end:.2f} USDT | "
        f"PnL: {pnl_end:.2f} USDT"
    )
    version_label_end = f'v{BOT_VERSION}'
    if not IS_LATEST_VERSION:
        version_label_end += f' (latest {EXPECTED_VERSION})'
    end_banner = f"{'=' * 18} SESSION END {end_stamp} {version_label_end} {'=' * 18}"
    log(end_banner, Fore.MAGENTA)
    if not IS_LATEST_VERSION:
        log(f"?? Running fallback build {version_label_end}.", Fore.YELLOW)
    send_tg(f"=========== END SESSION {end_stamp} ===========")
    send_tg(f"?? {end_balance_text}")
    info_tail = "See changelog in launch notification" if CHANGELOG_TEXT else "No changelog entry"
    send_tg(f"ℹ️ Version {version_label_end}. {info_tail}")
