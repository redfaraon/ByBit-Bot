...
3. Export environment variables (example):
   ...
   export TELEGRAM_COMMANDS="status:“екущий статус;help:—правка"  # slash-command overrides
   export NEWS_PROVIDER=hybrid                   # news sources: hybrid (default), cryptocompare, rss
   ```
### Telegram diagnostics

- `/ai payload [context]` Ч dumps the last OpenAI request/response snapshot (pass `universe` or `trade` to focus on a stage; without arguments the latest exchange is shown).
- `/logs [count]` Ч prints the latest console log lines (default 20, max 200) directly in Telegram.
...
- Mixed mode is supported on a unified account:
  ...
  - The bot skips УopenФ short on spot (`SELL` to open). Closing spot exposure is done by explicit `SELL` of held assets.
