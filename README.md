...
- **Prod (????)** ? ???????????? default, ????? ? users/default/secrets.env, ??? ??????.
- **Testnet (sandbox)** ? ????????? ???????????? (???????? demo), ????? ???????? ? users/demo/secrets.env, ?????? BYBIT_SANDBOX=1 (??? BYBIT_TESTNET=1).
- **Demo Trading (api-demo.bybit.com)** ? ????????? ????-???? ? ????????? ???????????. ????? Demo Trading ?????? ? users/demo/secrets.env, ?????? BYBIT_DEMO=1 (??? BYBIT_ENV=demo); BYBIT_API_BASE=https://api-demo.bybit.com ??????????? ????????????? ??? BYBIT_DEMO=1, ????? ?????? ???????.
...
# demo/testnet (??????? ?? ????? ? users/demo/secrets.env)
python3 bybitbot.py --user demo
`
???: 	ail -f assets/bybit.log ??? 	ail -f runtime/<user>/bybit.log.

### 3) ????????? systemd-???? ??? demo/testnet/api-demo
...
- /demo ????????? ybitbot-demo.service (????? ?????????????? BYBITBOT_DEMO_SERVICE).
- /bybitkey <apiKey> <apiSecret> ????????? ????? ? users/<user>/secrets.env ?? ????????? ????????????.
