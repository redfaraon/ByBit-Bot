...
def _safe_round(value: Optional[float], digits: int = 8) -> Optional[float]:
    return protection_engine._safe_round(value, digits=digits)


def _extract_protection_orders(orders) -> list[dict[str, Any]]:
    return protection_engine._extract_protection_orders(orders)


def _categorize_protection_orders(orders) -> dict[str, list[tuple]]:
    return protection_engine._categorize_protection_orders(orders)
...
def _evaluate_position_protection(
    ...
    return protection_engine._evaluate_position_protection(
        position_payload,
        orders,
...
def _describe_protection_changes(initial_orders, final_orders) -> list[str]:
    return protection_engine._describe_protection_changes(initial_orders, final_orders)


def _format_protection_snapshot(orders, position_payload: dict | None = None) -> str:
    return protection_engine._format_protection_snapshot(orders, position_payload)
...
def _select_best_protection_levels(
    ...
    return protection_engine._select_best_protection_levels(position_payload, orders)


def _format_progress_to_levels(progress: dict[str, Any]) -> str:
    return protection_engine._format_progress_to_levels(progress)


def _format_close_reason(reason: str | None) -> str:
    return protection_engine._format_close_reason(reason)


def _get_position_reference_price(payload: dict | None) -> float | None:
    return protection_engine._get_position_reference_price(payload)


def _protection_orders_signature(orders) -> tuple:
    return protection_engine._protection_orders_signature(orders)
...
def _cleanup_redundant_stop_orders(
    ...
    return protection_engine._cleanup_redundant_stop_orders(
        exchange,
        symbol,
        reduce_orders,
        protection_side,
...
def _close_position_now(
    ...
    return protection_engine._close_position_now(
        exchange,
        symbol,
        close_side,
        qty,
...
def _trail_state_matches_position(trail_state: dict | None, position_payload: dict | None) -> bool:
    return protection_engine._trail_state_matches_position(trail_state, position_payload)


def ensure_position_protection(exchange, symbol, position, df_primary, open_orders, config=None):
    return protection_engine.ensure_position_protection(
        exchange,
        symbol,
        position,
        df_primary,
...
def _prepare_protection_dataframe(
    ...
    return protection_engine._prepare_protection_dataframe(df_candidate, df_primary, symbol)
...
def _refresh_position_protection_if_possible(
    ...
    return protection_engine._refresh_position_protection_if_possible(
        exchange,
        symbol,
        position,
        df_candidate,
...
def run_cycle():
    ...
    price_hints: dict[str, float | None] = {}
    for sym_active, payload in final_positions_map.items():
        ...
        price_hints[str(sym_active)] = px_val
    ...
    for sym_unprotected, amount_unprotected in unprotected_positions:
        ...
        px_val = price_hints.get(str(sym_unprotected))
        if px_val is None or not math.isfinite(px_val):
            mark_price = safe_float(
                (position_payload.get("markPrice") if isinstance(position_payload, dict) else None)
                or (position_payload.get("raw") or {}).get("markPrice") if isinstance(position_payload, dict) else None
            )
            last_price = safe_float(
                (position_payload.get("lastPrice") if isinstance(position_payload, dict) else None)
                or (position_payload.get("raw") or {}).get("lastPrice") if isinstance(position_payload, dict) else None
            )
            px_val = mark_price if mark_price is not None and math.isfinite(mark_price) else last_price
        try:
